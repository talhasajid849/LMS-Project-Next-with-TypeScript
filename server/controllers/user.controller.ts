        ...
        try {
            ...
            try {
                await sendMail({
                    ...
                    template: "acivation-mail.ejs",
                    data,
                });

                res.status(201).json({
                    success: true,
                    message: `Please check your email: ${user.email} to activate your acoount`,
...
export const activateUser = CatchAsyncError(
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if (exitsUser) {
                return next(new ErrorHandler("Emnail already exists", 400));
            }
...
export const updateUserInfo = CatchAsyncError(
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            // ✅ Ensure ID is valid and convert to string for Redis
            if (!userId) {
                return next(new ErrorHandler("User not authenticated", 401));
            }
            ...
            await redis.set(idString, JSON.stringify(user));
...
export const updatePassword = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const idString = userId instanceof Types.ObjectId ? userId.toString() : userId;
        const user = await userModel.findById(idString).select("+password");
...
// Delete the user --- for Admin only
